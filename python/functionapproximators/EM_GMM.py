...
def em_GMM(inputs_GMM,initial_means, initial_covariance, initial_priors):
    ...
    for em in range(em_iter):
        for num_iter in range(max_iter):
            ...
            l[num_iter] = np.mean(np.log(np.sum(mvg,0)))

            if num_iter > min_iter:
                # print(l[num_iter]-l[num_iter-1],num_iter)
                if (l[num_iter]-l[num_iter-1])<converge:
                    print(num_iter)
